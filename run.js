...
`;
    ...
    // ★ v3.5.2: 기존엔 supportsTouch로 마우스/터치 이벤트를 분기했는데,
    //   일부 PC(터치스크린 노트북, 정밀 트랙패드 등)가 maxTouchPoints > 0으로 잡히면서
    //   마우스 클릭엔 반응 안 하는 버그가 있었음(버튼은 보이는데 눌러도 무반응).
    //   Pointer Events로 통합해서 마우스든 터치든 펜이든 하나의 경로로 처리.
    if (window.PointerEvent) {
        btnEl.addEventListener("pointerdown", (e) => {
            onDragStart(e);
            try { btnEl.setPointerCapture(e.pointerId); } catch { /* ignore */ }
        });
        window.addEventListener("pointermove", onDragMove);
        window.addEventListener("pointerup", onDragEnd);
        window.addEventListener("pointercancel", () => { dragging = false; btnEl.classList.remove("dragging"); });
